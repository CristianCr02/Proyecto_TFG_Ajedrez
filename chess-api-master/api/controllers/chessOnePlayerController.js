    ...
    getChess(gameId, currentGame => {
        if (currentGame != null) {
            ...
            if (chess != null) {
                ...
                var move;
                if (promoted) {
                    move = chess.move({ from: fromSq, to: toSq, promotion: promoted });
                } else {
                    move = chess.move({ from: fromSq, to: toSq });
                }
                // ------------------------------------------------
                // var move = chess.move({ from: fromSq, to: toSq }); // DELETED
...
/**
 * Retrieves the moves for a specific game
 * @param {*} req 
 * @param {*} res 
 * @returns 
 */
exports.getAllMoves = async (req, res) => {
    ...
    try {
        const moves = await ChessGame.findOne({ game_id: gameId }, { chess_moves: 1, _id: 0 });
        if (!moves) {
            status.status = "error: The game has expired OR you didn't put the game_id as the parameter!";
